...
# Suppress Qt warnings in Termux
...
class PhiAVDilation:
    ...
    def video_thread(self):
        ...
        if not cap.isOpened():
            print("⚠️ Camera not available — running audio-only mode")
            return
        print("📹 Camera opened successfully")
        while True:
            ret, frame = cap.read()
            if not ret:
                print("⚠️ Frame read failed — stopping video thread")
                break
    ...
    def audio_callback(self, indata, frames, time_info, status):
        """Fixed signature for sounddevice callback"""
        if status:
    ...
    def run(self):
        print("🎬 PHI A/V DILATION ACTIVE — Termux Edition")
        print("Speak into mic — audio will dilate with flux")
        print("Camera optional — audio works standalone")

        # Start video thread (non-blocking)
            ...
            with sd.Stream(samplerate=self.audio_sr, blocksize=1024, callback=self.audio_callback):
                ...
                while True:
                    ...
                    try:
                        ...
                        self.video_history.append(frame.copy())
                    ...
                    # Show frame (headless-safe)
                    try:
                        cv2.imshow("PHI VISOR", display)
                        if cv2.waitKey(1) == ord('q'):
                            break
                    except:
                        pass  # No display — run headless

                    # Audio playback
                    try:
                        audio = self.audio_q.get_nowait()
                        self.audio_history.append(audio.copy())
