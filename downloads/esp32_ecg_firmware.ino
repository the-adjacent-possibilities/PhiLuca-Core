...
# Save: Ctrl+X, Y, Enter/* Φ-LUCA Bio-Instrument v1.0 - REAL ECG Stream to WebSocket */
#include <WiFi.h>
#include <WebSocketsServer.h>
#include <ArduinoJson.h>

const char* ssid = "YOUR_WIFI";           // ← CHANGE THIS
...
void setup() {
  ...
  Serial.println("
Φ-LUCA ECG Linked: " + WiFi.localIP().toString());
  webSocket.begin();
...
void loop() {
  ...
  if (now - lastSample >= (1000000 / SAMPLE_RATE_HZ)) {
    int ecgValue = analogRead(ECG_PIN);
    ecgBuffer[bufferIndex] = ecgValue;
    bufferIndex = (bufferIndex + 1) % 100;
    
    // Stream JSON packet every 10 samples (50Hz UI update)
    if (bufferIndex % 10 == 0) {
